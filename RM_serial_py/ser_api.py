...
# 构建双倍易伤次数数据包 
def build_data_double_effect_times(target_robot_id, double_times, state):
    data = bytearray()
    data_cmd_id = bytearray([0x04, 0x02])  # 子命令ID为0x0204（小端顺序）
    
    # 发送者ID为雷达ID (红方为9，蓝方为109)
    sender_id = 9 if state == 'R' else 109
    data.extend(bytearray(struct.pack('H', sender_id)))
    data.extend(bytearray(struct.pack('H', target_robot_id)))
    data.extend(bytearray([double_times]))
    
    return data_cmd_id + data
