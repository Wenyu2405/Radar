...
# from mind.mind_camera import MindCamera
from hik.hik_camera import call_back_get_image, start_grab_and_get_data_size, close_and_destroy_device, set_Value, \
...
def dahua_camera_get():
    ...
    # 从配置文件读取相机参数
    camera_params = config['camera_params']['dahua']['camera1']
    exposure_time = camera_params['exposure_time']  # 曝光时间，单位：微秒
    gain = camera_params['gain']           # 增益值
    gamma_value = camera_params['gamma']     # 伽马值
...
def mind_camera_get():
    ...
    if not cam.init_sdk():
        print("迈德相机SDK初始化失败")
        return
    devices = cam.enum_devices()
    if not devices:
        print("未找到迈德相机设备")
        return
        
    if not cam.open_camera(0):  # 打开第一个相机
        print("打开迈德相机失败")
        return
        
    print("迈德相机已打开，开始采集图像")
    
    # 从配置文件读取相机参数
    camera_params = config['camera_params']['mind']
    exposure_time = camera_params['exposure_time']
    gain = camera_params['gain']
    gamma = camera_params['gamma']
    
    # 这里应该设置相机参数，但原代码没有实现，保持不变
    
    while True:
        try:
            img = cam.get_frame(1000)  # 超时时间1000ms
            if img is not None:
                camera_image = img
                time.sleep(0.016)  # 约60fps
        except Exception as e:
        ...
        # 检查是否需要退出循环
        if not getattr(cam, 'camera_initialized', False):
            break
...
class MyUI(QWidget):
    ...
    def initUI(self):
        ...
        self.T = []
        if self.state == 'R':
            self.save_path = config['paths']['calibration']['red']
            right_image_path = config['paths']['map_images']['red']
        else:
            self.save_path = config['paths']['calibration']['blue']
            right_image_path = config['paths']['map_images']['blue']
...
if __name__ == '__main__':
    camera_mode = config['global']['camera_mode']  # 'test':测试模式, 'dahua':大华相机, 'hik':海康相机, 'mind':迈德相机, 'video':USB相机
    camera_image = None
    state = config['global']['state']  # R:红方/B:蓝方

    if camera_mode == '24test':
        camera_image = cv2.imread('images/test_image.jpg')

    elif camera_mode =='test':
        camera_image =cv2.imread('images/253Dtest.png')
        ...
        thread_camera.start()
    elif camera_mode == 'mind':
        # 迈德相机图像获取线程
        thread_camera = threading.Thread(target=mind_camera_get, daemon=True)
        thread_camera.start()
