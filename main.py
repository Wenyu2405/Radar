...
from RM_serial_py.ser_api import build_send_packet, receive_packet, Radar_decision, \
    build_data_decision, build_data_radar_all, build_data_double_effect_times
import yaml

import rclpy
from livox.livox_handler import LivoxHandler
...
livox_points_count = 0
livox_is_receiving = False
# 在配置文件加载后初始化ROS2和Livox雷达
use_livox = config['global'].get('use_livox', False)
if use_livox:
    # 初始化ROS2
    rclpy.init(args=None)
    livox_handler = LivoxHandler()
    
    # 创建ROS2线程以处理回调
    def ros2_spin():
        while True:
            rclpy.spin_once(livox_handler, timeout_sec=0.1)
    
    ros2_thread = threading.Thread(target=ros2_spin, daemon=True)
    ros2_thread.start()
    
    print("Livox雷达数据接收已初始化")
...
recording_config = config.get('recording', {
    'enabled': True,             # 默认开启录制功能
    'path': 'recordings',        # 默认保存路径
    'fps': 30,                   # 默认帧率
    'codec': 'XVID',             # 默认编解码器
    'resolution': None,          # 默认使用原始分辨率
    'auto_start': True,          # 默认自动开始录制
    'record_raw': True,          # 默认录制原始帧
    'record_processed': True,    # 默认录制处理后帧
    'max_duration_minutes': 0,   # 默认不限制单个文件录制时长
    'file_prefix': 'robomaster_' # 默认文件名前缀
})

# 初始化录制器
recorder = None
recording_start_time = 0
max_recording_duration = recording_config.get('max_duration_minutes', 0) * 60  # 转换为秒

if recording_config.get('enabled', True):  # 默认启用
    resolution = recording_config.get('resolution')
    if resolution:
        resolution = tuple(resolution)  # 将列表转换为元组
    
    # 创建录制器
    recorder = VideoRecorder(
        output_path=os.path.join(recording_config['path'], 'camera'),
        fps=recording_config['fps'],
        codec=recording_config['codec'],
        resolution=resolution
    )
    print("相机录制已初始化，将在第一帧可用时自动开始")
    
    recording_start_time = time.time()
    print("视频录制已初始化。按'r'键开始/停止录制")


def dahua_camera_get():
    global camera_image
    ...
    # 从配置文件读取相机参数
    camera_params = config['camera_params']['dahua']
    exposure_time = camera_params.get('exposure_time', 16000)  # 曝光时间，单位：微秒
    gain = camera_params.get('gain', 17.9)           # 增益值
    gamma_value = camera_params.get('gamma', 1.0)     # 伽马值
    
    if camera.set_exposure_time(exposure_time):
        print(f"设置相机曝光时间为 {exposure_time} 微秒")

    if camera.set_gain(gain):
        print(f"设置相机增益为 {gain}")

    if camera.set_gamma(gamma_value):
        print(f"设置相机伽马值为 {gamma_value}")
    ...
    try:
        ...
        while True:
            # 获取图像
            img = camera.get_frame(timeout_ms=1000)
...
def ser_send():
    ...
    last_double_effect_time = 0 # 记录上一次发给云台手次数的时间
    ...
    while True:
        ...
        try:
            ...
            # 双倍易伤次数发送给云台手
            current_time = time.time()
            if current_time - last_double_effect_time > 1.0:  # 每秒发送一次
                if double_vulnerability_chance >= 0:  # 如果有双倍易伤次数信息
                    # 向己方步兵发送双倍易伤次数
                    if state == 'R':
                        soldier_ids = [6]  # 红方步兵ID
                    else:
                        soldier_ids = [106]  # 蓝方步兵ID
                    
                    for soldier_id in soldier_ids:
                        # 构建双倍易伤次数数据包
                        data = build_data_double_effect_times(soldier_id, 
                    ...
                    print(f"已发送双倍易伤次数 {double_vulnerability_chance} 到己方步兵")
                
                last_double_effect_time = current_time#更新

            # 判断飞镖的目标是否切换，切换则尝试发动双倍易伤
            print(f"当前状态: 机会数={double_vulnerability_chance}, 易伤状态={opponent_double_vulnerability}, 目标={target}, 上一目标={target_last}")
            if target != 0:
                target_last = target
                # 有双倍易伤机会，并且当前没有在双倍易伤
...
if camera_mode == 'test':
    ...
    thread_camera.start()
elif camera_mode == 'dahua':
    # 大华相机图像获取线程
    thread_camera = threading.Thread(target=dahua_camera_get, daemon=True)
    thread_camera.start()
...
while camera_image is None:
    ...
    time.sleep(0.5)
# 获取第一帧后，立即开始录制
if recorder and recording_config.get('auto_start', True):
    recorder.start(camera_image)
...
while True:
    ...
    # 检查是否需要重新开始录制（如果设置了最大持续时间）
    if max_recording_duration > 0 and (current_time - recording_start_time) > max_recording_duration:
        print(f"已达到最大录制时间 ({max_recording_duration} 秒)，重新开始新文件")
        
        # 停止当前录制并开始新的录制
        if recorder and recorder.is_recording:
            recorder.stop()
            recorder.start(camera_image.copy())
    ...
    # 确保录制一直在进行
    if recorder and recording_config.get('auto_start', True) and not recorder.is_recording:
        recorder.start(img0)
    ...
    system_status = {
        'team_color': state,
        'vulnerability_chances': double_vulnerability_chance,
        'vulnerability_active': "Active" if opponent_double_vulnerability == 1 else "Inactive" if opponent_double_vulnerability == 0 else "Unknown",
        'recording_status': "Recording" if (recorder and recorder.is_recording) else "Stopped",
        'camera_status': "Connected" if camera_image is not None else "Disconnected",
        'radar_status': "Connected" if (use_livox and livox_is_receiving) else "Disconnected" if use_livox else "Disabled",
        'fps': fps
    }

# 绘制UI（传入系统状态）
    _ = draw_information_ui(vulnerability, state, information_ui_show, system_status)
    ...
    # cv2.putText(information_ui_show, "vulnerability_Triggeri  (opponent_double_vulnerability),
    #             (10, 400),
    #             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    # cv2.putText(information_ui_show, "FPS: {:.2f}".format(fps),
    #             (10, 450),
    #             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
# 显示点云信息 - 简化版
    # if use_livox:
    # # 显示雷达状态
    #     status_text = "open" if livox_is_receiving else "close"
    #     status_color = (0, 255, 0) if livox_is_receiving else (0, 0, 255)
    
    #     cv2.putText(information_ui_show, f"radar_state: {status_text}", 
    #             (10, 500), 
    #             cv2.FONT_HERSHEY_SIMPLEX, 0.7, status_color, 2)
    
    # # 只显示点数
    #     cv2.putText(information_ui_show, f"point_cloud_number: {livox_points_count}", 
    #             (10, 550), 
    #             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
...
# 点云信息处理
    if use_livox:
        if livox_handler.is_receiving_data():
            livox_is_receiving = True
            livox_vis = livox_handler.get_visualization()
            livox_vis_resized = cv2.resize(livox_vis, (600, 400))
            cv2.imshow('Livox Radar', livox_vis_resized)
            if livox_handler.latest_point_cloud is not None:
            # 只获取点数量
                points = livox_handler.extract_points(livox_handler.latest_point_cloud)
                if points is not None:
                    livox_points_count = len(points)
    ...
    # 在information_ui_show上添加录制状态
    if recorder and recorder.is_recording:
        cv2.putText(information_ui_show, "recording。。。", 
                    (10, 500),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    
    # 显示录制时间
    if recorder and recorder.is_recording:
        elapsed_time = int(current_time - recording_start_time)
        minutes = elapsed_time // 60
        seconds = elapsed_time % 60
        cv2.putText(information_ui_show, f"recording time: {minutes:02d}:{seconds:02d}", 
                    (10, 550),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
